...
import { fetchHaa, searchHaa } from '../../services/haaData';
...
<template>
    <div class="min-h-screen bg-slate-950/5 py-6">
        ...
        <div class="mx-2 mt-5">
            <div v-if="filteredHaa.length === 0" class="rounded-[28px] bg-white/90 border border-slate-200/20 p-8 text-center text-slate-600">
                No results found.
            </div>

            <div v-else class="grid gap-1 md:grid-cols-2 xl:grid-cols-3">
                <div v-for="song in filteredHaa" :key="song.id" class="group">
                    <RouterLink :to="`/haa/show/${song.id}`" class="block overflow-hidden rounded-[28px] border border-slate-200/10 bg-white/90 py-1 shadow-sm transition hover:-translate-y-1 hover:shadow-xl hover:bg-white">
